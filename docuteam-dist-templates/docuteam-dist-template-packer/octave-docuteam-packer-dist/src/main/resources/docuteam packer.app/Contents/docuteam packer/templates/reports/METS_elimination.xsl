...
<xsl:stylesheet version="2.0"
	...
	<xsl:variable name="size-of-all-files-trash">
		<xsl:variable name="files-trash">
			<xsl:for-each select="//METS:div[@TYPE = 'file']">
				<xsl:variable name="dmd-id" select=".//METS:div[@LABEL = 'EAD']/@DMDID"/>
				<xsl:if test="//METS:dmdSec[@ID = $dmd-id]//EAD:c/@otherlevel = 'Trash'">
					<xsl:variable name="adm-id" select="./@ADMID"/>
					<file-size><xsl:value-of select="(//METS:digiprovMD[@ID = $adm-id]//PREMIS:size)[last()]"/></file-size>
				</xsl:if>
			</xsl:for-each>
		</xsl:variable>
		<xsl:value-of select="sum($files-trash/file-size)"/>
	</xsl:variable>
	
	<xsl:variable name="sorted-dates">
		<xsl:perform-sort select="//EAD:c[@otherlevel = 'Trash']//EAD:unitdate[@label = 'to' or @label = 'from']">
	...
	<xsl:variable name="number-of-files-trash">
		<xsl:variable name="files-trash">
			<xsl:for-each select="//METS:div[@TYPE = 'file']">
				<xsl:variable name="dmd-id" select=".//METS:div[@LABEL = 'EAD']/@DMDID"/>
				<xsl:if test="//METS:dmdSec[@ID = $dmd-id]//EAD:c/@otherlevel = 'Trash'">
		...
		<xsl:value-of select="count($files-trash/file)"/>
	</xsl:variable>
	
	<xsl:variable name="number-of-folders-trash">
		<xsl:variable name="folders-trash">
		...
		<xsl:value-of select="count($folders-trash/folder)"/>
	</xsl:variable>
				...
				<fo:flow flow-name="body" font-size="10pt">
					<fo:block-container>
						<fo:block font-size="15pt" font-weight="bold" text-align="center" space-after="15pt">
							<xsl:text>Bordereau d'élimination d'archives publiques</xsl:text>
						</fo:block>
								...
								<fo:table-body >
									...
									<fo:table-row border-after-style="solid">
										<fo:table-cell number-columns-spanned="2" padding="5pt">
											<fo:block>
												<xsl:text>Nombre total d'articles (fichiers, dossiers) dont l'élimination est demandée : </xsl:text>
											</fo:block>
											<fo:block>
												<xsl:text>Fichiers : </xsl:text><xsl:value-of select="$number-of-files-trash"/>
											</fo:block>
											<fo:block>
												<xsl:text>Dossiers : </xsl:text><xsl:value-of select="$number-of-folders-trash"/>
											</fo:block>
										</fo:table-cell>
									</fo:table-row>
									<fo:table-row  border-after-style="solid">
									...
									<fo:table-row border-after-style="solid">
										<fo:table-cell number-columns-spanned="2" padding="5pt">
											<fo:block>
												<xsl:text>Poids total des documents dont l'élimination est demandée (en kilo-octets) : </xsl:text>
											</fo:block>
											<fo:block>
												<xsl:value-of select="format-number($size-of-all-files-trash div 1024, '###''###', 'european')"/><xsl:text> kilo-octets</xsl:text>
											</fo:block>
										</fo:table-cell>
									</fo:table-row>
									<fo:table-row>
										<fo:table-cell number-columns-spanned="2" padding="5pt">
											<fo:block>
												<xsl:text>Motif de l'élimination : (à compléter par l'archiviste)</xsl:text>
											</fo:block>
										</fo:table-cell>
									</fo:table-row>
									<fo:table-row>
								...
								<fo:table-body>
									<fo:table-row font-weight="bold">
										<fo:table-cell border="solid" padding="5pt">
											<fo:block>
												<xsl:text>Nom et URI des fichiers à éliminer</xsl:text>
											</fo:block>
											<fo:block>
												<xsl:text>Nom et URI des dossiers à éliminer et nombre de fichiers contenus</xsl:text>
											</fo:block>
										</fo:table-cell>
										<fo:table-cell border="solid" padding="5pt">
											<fo:block>
