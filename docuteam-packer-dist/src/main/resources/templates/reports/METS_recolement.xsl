...
<xsl:stylesheet version="2.0"
	...
	<xsl:variable name="sorted-dates">
		<xsl:perform-sort select="//EAD:did/EAD:unitdate[@label = 'to' or @label = 'from']">
	...
	<xsl:variable name="number-of-files">
		<xsl:value-of select="count(//METS:div[@TYPE='file'])"/>
	</xsl:variable>
	<xsl:variable name="number-of-tracabilite-files">
		<xsl:value-of select="count(//METS:div[@LABEL='Traçabilité des opérations réalisées pendant le traitement']/METS:div[@TYPE='file'] )" />
	</xsl:variable>
	<xsl:variable name="number-of-files-minus-tracabilite">
		<xsl:value-of select="$number-of-files - $number-of-tracabilite-files"/>
	</xsl:variable>
				...
				<fo:flow flow-name="body" font-size="10pt">
					<fo:block-container>
						<fo:block font-size="15pt" font-weight="bold" text-align="center" space-after="15pt">
							<xsl:text>Bordereau de récolement d'archives</xsl:text>
						</fo:block>
								...
								<fo:table-body >
									...
									<fo:table-row border-after-style="solid">
										<fo:table-cell number-columns-spanned="2" padding="5pt">
											<fo:block>
												<xsl:text>Nombre total de fichiers contenu dans le versement : </xsl:text>
											</fo:block>
											<fo:block>
												<xsl:value-of select="$number-of-files-minus-tracabilite"/>
											</fo:block>
										</fo:table-cell>
									</fo:table-row>
									<fo:table-row  border-after-style="solid">
									...
									<fo:table-row border-after-style="solid">
										<fo:table-cell number-columns-spanned="2" padding="5pt">
											...
											<fo:block>
												<xsl:value-of select="format-number($total-size div 1024, '###''###', 'european')"/><xsl:text> kilo-octets</xsl:text>
											</fo:block>
										</fo:table-cell>
									</fo:table-row>
									<fo:table-row>
								...
								<fo:table-body>
									<fo:table-row font-weight="bold">
										<fo:table-cell border="solid" padding="5pt">
											<fo:block>
												<xsl:text>Nom du fichier / URI / Empreinte</xsl:text>
											</fo:block>
										</fo:table-cell>
										<fo:table-cell border="solid" padding="5pt">
											<fo:block>
		...
		<fo:table-row display-align="before" border="solid">
			<fo:table-cell padding="5pt">
				<fo:block-container page-break-inside="avoid">
					...
					<fo:block>
						<xsl:call-template name="zero_width_space_1">
							<xsl:with-param name="data" select="(//METS:digiprovMD[@ID = $adm-id]//PREMIS:messageDigest)[last()]"/>
						</xsl:call-template>
					</fo:block>
				</fo:block-container>
			</fo:table-cell>
