...
/**
 * Shows duplicates with the same checksum in a table, with different checksums in different tables.
 * Remove duplicates.
 * 
 * @author l.dumitrescu
 *
...
public class DuplicatesTableDialog extends JDialog {
	...
	public DuplicatesTableDialog(NodeAbstract nodeAbstract, SIPView sipView) {
		...
		GridBagPanel gridBag = new GridBagPanel(new EmptyBorder(1, 1, 1, 1), new Insets(5, 5, 5, 5));
		gridBag.add(new JLabel(I18N.translate("LabelSelectDuplicateForRemoval")), 1, 1, 1, 4, GridBagConstraints.WEST);
						
		GridBagPanel innerGridBag = new GridBagPanel(new BevelBorder(1), new Insets(1, 1, 1, 1));
		
		//for each checksum add a table
		Iterator<String> keyIterator = duplicatesListMultimap.keySet().iterator();
		int rowIndex = 1;
		while(keyIterator.hasNext()) {			
			String nextKey = keyIterator.next();
			List<DuplicateNodeFile> valueList = duplicatesListMultimap.get(nextKey);			
			DuplicatesTable currentTable = new DuplicatesTable(valueList, false);			
			tableList.add(currentTable);
			innerGridBag.add(currentTable, rowIndex, rowIndex, 1, 1, GridBagConstraints.NORTH, GridBagConstraints.NONE, 1, 0.1);			
			rowIndex++;
		}
		gridBag.add(new JScrollPane(innerGridBag), 2, 2, 1, 4, GridBagConstraints.NORTH, GridBagConstraints.BOTH, 1, 0.1);
	...
	private void removeDuplicateButtonClicked() {		
		...
		for(DuplicatesTable table:tableList) {
