...
public class TestOrAssignSADialog extends JDialog {
    ...
    protected TestOrAssignSADialog(final SIPView sipView) {
        ...
        final GridBagPanel gridBag = new GridBagPanel(new EmptyBorder(10, 10, 10, 10), new Insets(5, 5, 5, 5));
    ...
    protected void saSelected() {
        ...
        final List<NodeFile> badFiles = sipView.getDocument().filesNotAllowedBySubmissionAgreement(selectedSA,
    ...
    protected void setSAButtonClicked() {
        ...
        try {
            badFiles = sipView.getDocument().setSubmissionAgreement(selectedSAOverview.saId,
                    selectedSAOverview.dssId);
        } catch (final Exception ex) {
            Logger.error(ex.getMessage(), ex);
        }
    ...
    protected void deleteAllButtonClicked() {
        for (final NodeFile nodeFile : ((BadFilesTableModel) badFilesTable.getModel()).getList()) {
            try {
                nodeFile.delete();
            } catch (final Exception ex) {
                Logger.error(ex.getMessage(), ex);
    ...
    protected List<Overview> getAllFinalSAOverviews() {
        final List<Overview> saOverviewList = SubmissionAgreement.getAllFinalOverviews();
        ...
        for (final Overview overview : saOverviewList) {
            if (overview != null && overview.saId.equals(sipView.getDocument().getSAId()) && overview.dssId.equals(
                    sipView.getDocument().getDSSId())) {
